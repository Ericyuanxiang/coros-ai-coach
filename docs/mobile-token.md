...
> Coros APK, `coros-ai-coach auth` now obtains a mobile API token automatically with no
...
> In normal operation you don't need to do anything with this token — `coros-ai-coach auth`
...
The mobile token expires after approximately 1 hour. `coros-ai-coach auth` stores the
